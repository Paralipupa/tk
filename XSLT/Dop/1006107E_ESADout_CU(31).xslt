 ...
 -->
		...
		<div class="page" style="min-height:297mm">
			...
			<xsl:for-each select="//ESADout_CU:ESADout_CUGoods">
				<xsl:variable name="numRec" select="position()"/>
					<br/>------ Товар № <xsl:value-of select="position()"/> ------<br/>
	...
	<xsl:template name="graf_31_3_container">
		...
		<xsl:if test="ESADout_CU:ESADContainer">
			...
			<xsl:variable name="row_count" select="count(//catESAD_cu:ContainerNumber)"/> <!-- Polaris кол-во кнтейнеров -->
			
				<xsl:for-each select="ESADout_CU:ESADContainer">
					<xsl:if test="$backSide=0">
						<xsl:apply-templates select="catESAD_cu:ContainerQuantity"/>
						<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
						<xsl:apply-templates select="catESAD_cu:ContainerKind"/>:
					</xsl:if>
					...
					<xsl:if test="position() = $max_count+1 and $backSide=0"> <!-- Polaris -->
						СМ.ДОПОЛНЕНИЕ
					</xsl:if>
				</xsl:for-each>
	...
	<xsl:template name="graf_31_10_CIMI">
		...
		<xsl:if test="catESAD_cu:CIMIdDetails">
			<xsl:variable name="max_count" select="5"/> <!-- Polaris максимальное кол-во -->
			<xsl:variable name="row_count" select="//catESAD_cu:CIMQuantity"/> <!-- Polaris кол-во-->			
			<br/>
			<span class="graph">10) </span>
			<xsl:if test="($row_count &lt;= $max_count and $backSide=0) or ($backSide!=0)">
			...
			<xsl:if test="$row_count > $max_count and $backSide=0">
				СМ.ДОПОЛНЕНИЕ
			</xsl:if>
		</xsl:if>
	...
	<xsl:template name="backTD4Head">
		<table style="width: 100%; vertical-align: top; border-collapse: collapse; font-size: 7pt;">
			<tr>
				...
				ДОПОЛНЕНИЕ НА ______ Л., К ДТ N________________________________
				</td>
			</tr>
		</table>
		<br/>
